...
export async function getHGroupSummary(): Promise<HGroupSummary[]> {
  ...
  //
  // NB: do NOT switch this to cys_classification.hgroup_summary_struct.
  // That sibling MV uses cysteine_classifications.evidence_class on the
  // PDB axis (true ground-truth × ESM2-predicted), which produces
  // 57 disulfide / 2 metal candidate-novel cells. The manuscript
  // headline (12 / 3) was generated by collaborators under the
  // ESM2-on-both-axes framing this MV provides; framing decision
  // recorded 2026-05-06 in DB_CONTRACT §3.3 to leave the page
  // numbers consistent with the manuscript. The struct MV stays
  // around for audit-only / future reproducibility queries.
  const rows = await query<{
    h_group_id: string;
    h_group_name: string;
    x_group_id: string;
