...
`hgroup_summary` but uses `evidence_class` on the PDB axis. **It is
intentionally unwired from the H-group browser** — `getHGroupSummary`
in `src/lib/queries.ts` keeps reading the original `hgroup_summary`
MV. Decision recorded 2026-05-06: the manuscript framing
(generated by collaborators) stands, and the page numbers should
match the manuscript headline ("12 disulfide / 3 metal
candidate-novel cells"). The sibling MV is retained for any future
audit or reproducibility query that needs the structurally-known
axis explicitly; do not flip the page over without first
re-coordinating with the manuscript authors.

Headline-cell counts (`<5%` known × `≥95%` predicted) on the v2 /
paper-v1 scope, for reference:

| Task | `hgroup_summary` (page) | `hgroup_summary_struct` (audit-only) | Manuscript |
| --- | --- | --- | --- |
| Disulfide | 10 H-groups | 57 H-groups | 12 |
| Metal | 1 H-group | 2 H-groups | 3 |
